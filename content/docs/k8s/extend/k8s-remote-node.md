...
你在自己电脑或本地机器上用 k3s/microk8s 跑了个 K8s 集群，但本机资源有限。想扩容跑点东西，于是去公有云买了台 2C4G 的小配置机器。

然后你发现：要把这台云主机加进集群，得在上面装 kubelet、containerd、配证书、开端口、改安全组……折腾半天，节点终于 Ready 了，但 kubelet 自己就吃了 500MB 内存——这台小机器总共才 4G。

你就想：**这机器上 Docker 已经有了，能不能直接 SSH 上去跑容器，省掉这些组件？**
...
你在自己电脑或本地机器上跑 k3s 当控制面，公有云上买几台低价小配置机器当 worker。这些机器不用装 kubelet，只保留 Docker + SSH，SSH 接进来就能跑 Pod。省下的资源全是你的。
...
借了台机器临时跑个数据迁移脚本，或者双十一紧急扩容——用完就删，零残留。
