...
想给集群加个节点跑几个 Pod，但机器可能是：

- 家里的 NAS 或树莓派
- 藏在 NAT 后面，没有公网 IP
- 一台跑着其他应用的服务器，不想装一堆 agent
- 临时借用的云主机

传统方案需要在目标机器上装 kubelet、containerd、配置证书、开一堆端口，搞不好还得搭 VPN。十几分钟折腾完发现防火墙规则不对，节点 NotReady……
...
家里有台配置不错的机器跑 Plex、Home Assistant？不用再装一套 K8s 全家桶，SSH 接进来就能跑批处理、定时任务、实验性 Pod。
...
双十一扩容、临时跑个数据迁移脚本、借了台朋友的机器测试——用完就删，零残留。

### 混合云

云上跑控制面，线下机器当 worker。不用拉专线也不用开端口，一条 SSH 搞定。
